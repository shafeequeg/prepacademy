...
import {
  ...
  // Offering,
  // OfferingType,
} from "@/app/collegecoursespage/Data";
...
const CUETExamApplySection: React.FC<CUETExamApplySectionProps> = ({
  ...
  useEffect(() => {
    ...
    // Add this: Set activeMainTab based on current course
    if (course) {
      // Find the tab that matches the current course
      const matchingTab = tabs.find((tab) =>
        tab.dropdownItems.some(
          (item) =>
            item.path.includes(slug) ||
            item.label.toLowerCase().includes(course.courseTitle.toLowerCase())
        )
      );
      if (matchingTab) {
        setActiveMainTab(matchingTab.id);
      }
    }
  }, [course, slug]); // Add slug to dependencies
