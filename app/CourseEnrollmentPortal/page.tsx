...
const CourseEnrollmentPortal: React.FC = () => {
  ...
  const handleEnrollNow = () => {
    ...
    router.push(`/payment/${uuid}?title=${encodeURIComponent(currentCourseData.title || activeCourse)}&price=${encodeURIComponent(currentCourseData.price)}&duration=${encodeURIComponent(currentCourseData.duration)}`);
  };
  ...
  const handleWishlistEnroll = (item: WishlistItem) => {
    const uuid = crypto.randomUUID();
    router.push(`/payment/${uuid}?title=${encodeURIComponent(item.title)}&price=${encodeURIComponent(item.price)}&duration=${encodeURIComponent(item.duration)}`);
    closeWishlistModal();
  };

  // const handleEnrollmentSuccess = () => {
  //   if (selectedCourse) {
  //     setEnrolledCourses([...enrolledCourses, `${activeMainTab}-${activeSubTab}-${activeCourse}`]);
  //     setEnrollmentSuccess(true);
  //     toast.success(`Successfully enrolled in ${activeCourse} Course!`);
  //   }
  // };
  ...
  //     features: courseMockData.features,
  //   };
  ...
  return (
    <div className="min-h-screen bg-[#2B1615] text-white">
      <header className="bg-[#5C2C1A] shadow-sm">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-white font-serif">
              Course Enrollment Portal
            </h1>
            <p className="text-orange-200 font-serif italic">
              Find and enroll in the best courses for your academic journey
            </p>
          </div>
          <button
            onClick={openWishlistModal}
            className="relative flex items-center gap-2 bg-orange-800 hover:bg-orange-700 text-white px-4 py-2 rounded-md transition-colors"
          >
            <Heart size={20} className="text-orange-200" />
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex flex-wrap border-b border-orange-800">
          {mainCategories.map((category) => (
            <button
              key={category.id}
              className={`px-4 py-3 text-sm font-medium ${
                activeMainTab === category.id
                  ? "text-white border-b-2 border-orange-500 bg-[#5C2C1A]"
                  : "text-orange-200 hover:text-white hover:bg-[#5C2C1A]"
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-4">
          <div className="bg-[#5C2C1A] rounded-lg shadow-md p-4 md:col-span-1 border border-orange-800">
            <h2 className="font-semibold text-orange-100 mb-4">Categories</h2>
            <div className="space-y-2">
              {activeMainCategory?.tabs.map((tab) => (
                <div key={tab.id} className="border-b border-orange-800 pb-2">
                  <div
                    className={`flex justify-between items-center py-2 px-3 rounded-md cursor-pointer ${
                      activeSubTab === tab.id
                        ? "bg-orange-800 text-white"
                        : "hover:bg-orange-900 text-orange-100"
                    }`}
                    onClick={() => {
                        ...
                        <div
                          ...
                          className={`py-1 px-3 text-sm rounded-md cursor-pointer ${
                            activeCourse === item.label
                              ? "bg-orange-800 text-white"
                              : "text-orange-200 hover:bg-orange-900"
                          }`}
          ...
          <div className="bg-[#5C2C1A] rounded-lg shadow-md p-6 md:col-span-3 border border-orange-800">
            {!activeSubTab ? (
              <div className="flex flex-col items-center justify-center py-16 bg-[#5C2C1A] rounded-lg">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {(activeMainTab === "schoolcourse" ||
                    ...
                    activeMainCategory?.tabs
                      ...
                      ?.dropdownItems?.map((course) => (
                        <div
                          key={course.path}
                          className="border border-orange-800 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer bg-[#5C2C1A]"
                          onClick={() => handleCourseClick(course.label)}
                        >
                          <div className="h-40 bg-orange-900"></div>
                          <div className="p-4">
                            ...
                            <div className="flex justify-between items-center mt-4">
                              ...
                              <button className="bg-orange-700 hover:bg-orange-600 text-white px-3 py-1 rounded-md text-sm">
                                Details
                              </button>
                            </div>
                          </div>
                  ...
                  {(activeMainTab === "studyabroad" ||
                    activeMainTab === "careercounseling") && (
                    <div
                      className="border border-orange-800 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer col-span-full bg-[#5C2C1A]"
                      onClick={() => {
                        const selectedTab = activeMainCategory?.tabs.find(
                          (tab) => tab.id === activeSubTab
                        );
              ...
              <div>
                <div className="mb-6">
                  <button
                    ...
                    className="text-orange-300 flex items-center text-sm hover:text-orange-100"
                  >
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="md:w-2/5">
                    <div className="h-64 bg-orange-900 rounded-lg"></div>
                  </div>
                  <div className="md:w-3/5">
                    ...
                    <div className="mt-4 flex items-center">
                      <div className="bg-orange-800 text-orange-100 px-2 py-1 rounded text-sm font-medium">
                        Bestseller
                      </div>
                      <div className="ml-3 text-orange-400 flex">
                        {[...Array(5)].map((_, i) => (
                          <span key={i}>★</span>
                        ))}
                      </div>
                    ...
                    <div className="mt-6 flex flex-col sm:flex-row gap-3">
                      <button 
                        ...
                        className="bg-orange-600 hover:bg-orange-500 text-white py-3 px-6 rounded-lg font-medium flex-1 transition-colors"
                      >
                        Enroll Now
                      </button>
                      <button 
                        onClick={addToWishlist}
                        className="border border-orange-600 text-orange-100 hover:bg-orange-900 py-3 px-6 rounded-lg font-medium transition-colors"
                      >
                        Add to Wishlist
                      </button>
                    </div>
                ...
                <div className="mt-10 border-t border-orange-800 pt-6">
                  ...
                  <p className="text-orange-200 mt-4">
                    ...
                    that you&apos;re always on track with your preparation. With our
                    structured approach and quality study materials, you&apos;ll be
                    well-prepared to achieve your academic goals.
                  </p>
                </div>
              </div>
            )}
